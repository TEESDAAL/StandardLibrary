...
interface Sk{
  String widgetKey="base.Sk.widgetKey";
  String textWarningKey="base.Sk.textWarningKey";
  String explicitSizeWarningKey="base.Sk.explicitSizeWarningKey";
  ...
  static void invalidateTree(java.awt.Component c){
    c.invalidate();
    if (c instanceof java.awt.Container p){
      for (var k: p.getComponents()){ invalidateTree(k); }
    }
  }

  static void paintNode(JComponent c,Canvas cv){
    ((AWidget<?,?>)c.getClientProperty(widgetKey)).sk(cv);
    for (var k: c.getComponents()){
      int save=cv.save();
      cv.translate(k.getX(),k.getY());
      cv.clipRect(Rect.makeWH(k.getWidth(),k.getHeight()));
      paintNode((JComponent)k,cv);
  ...
  static void text(Canvas cv,String text,AWidget<?,?> s,float dx,float dy){
    ...
    warnTextFit(c,text,s,(int)Math.ceil(textW),(int)Math.ceil(textH),cw,ch);
    if (cw<=0 || ch<=0){ return; }
    int save=cv.save();
  ...
  static void button(Canvas cv,_Button s){
    ...
    int center=baseColor(color(s.background),b.isEnabled(),over,down);
  ...
  private static int baseColor(int c,boolean enabled,boolean over,boolean down){
    if (c>>>24==0){ c=0xFFBEBEBE; }
    if (over){ c=mix(c,0xFFFFFFFF,12); }
    if (down){ c=mix(c,0xFF000000,16); }
    if (!enabled){ c=mix(c,0xFFA0A0A0,55); }
    return c;
  }
  ...
  static void warnTextFit(
    JComponent c,String text,AWidget<?,?> s,
    int textW,int textH,int availableW,int availableH
  ){}

  static void warnText(JComponent c,String msg){}

  static void warnExplicitSize(JComponent c,String msg){}
}